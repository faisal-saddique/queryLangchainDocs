`langchain/tools`
=================

Index[](#index "Direct link to Index")
---------------------------------------

### Classes[](#classes "Direct link to Classes")

*   [AIPluginTool](/docs/api/tools/classes/AIPluginTool)
*   [BingSerpAPI](/docs/api/tools/classes/BingSerpAPI)
*   [BraveSearch](/docs/api/tools/classes/BraveSearch)
*   [ChainTool](/docs/api/tools/classes/ChainTool)
*   [DadJokeAPI](/docs/api/tools/classes/DadJokeAPI)
*   [DataForSeoAPISearch](/docs/api/tools/classes/DataForSeoAPISearch)
*   [DynamicStructuredTool](/docs/api/tools/classes/DynamicStructuredTool)
*   [DynamicTool](/docs/api/tools/classes/DynamicTool)
*   [GoogleCustomSearch](/docs/api/tools/classes/GoogleCustomSearch)
*   [IFTTTWebhook](/docs/api/tools/classes/IFTTTWebhook)
*   [JsonGetValueTool](/docs/api/tools/classes/JsonGetValueTool)
*   [JsonListKeysTool](/docs/api/tools/classes/JsonListKeysTool)
*   [JsonSpec](/docs/api/tools/classes/JsonSpec)
*   [ReadFileTool](/docs/api/tools/classes/ReadFileTool)
*   [RequestsGetTool](/docs/api/tools/classes/RequestsGetTool)
*   [RequestsPostTool](/docs/api/tools/classes/RequestsPostTool)
*   [SearxngSearch](/docs/api/tools/classes/SearxngSearch)
*   [SerpAPI](/docs/api/tools/classes/SerpAPI)
*   [Serper](/docs/api/tools/classes/Serper)
*   [StructuredTool](/docs/api/tools/classes/StructuredTool)
*   [Tool](/docs/api/tools/classes/Tool)
*   [VectorStoreQATool](/docs/api/tools/classes/VectorStoreQATool)
*   [WikipediaQueryRun](/docs/api/tools/classes/WikipediaQueryRun)
*   [WriteFileTool](/docs/api/tools/classes/WriteFileTool)
*   [ZapierNLARunAction](/docs/api/tools/classes/ZapierNLARunAction)
*   [ZapierNLAWrapper](/docs/api/tools/classes/ZapierNLAWrapper)

### Interfaces[](#interfaces "Direct link to Interfaces")

*   [BraveSearchParams](/docs/api/tools/interfaces/BraveSearchParams)
*   [ChainToolInput](/docs/api/tools/interfaces/ChainToolInput)
*   [DataForSeoApiConfig](/docs/api/tools/interfaces/DataForSeoApiConfig)
*   [DynamicStructuredToolInput](/docs/api/tools/interfaces/DynamicStructuredToolInput)
*   [DynamicToolInput](/docs/api/tools/interfaces/DynamicToolInput)
*   [GoogleCustomSearchParams](/docs/api/tools/interfaces/GoogleCustomSearchParams)
*   [SerpAPIParameters](/docs/api/tools/interfaces/SerpAPIParameters)
*   [ToolParams](/docs/api/tools/interfaces/ToolParams)
*   [WikipediaQueryRunParams](/docs/api/tools/interfaces/WikipediaQueryRunParams)
*   [ZapierNLAWrapperParams](/docs/api/tools/interfaces/ZapierNLAWrapperParams)

### Type Aliases[](#type-aliases "Direct link to Type Aliases")

*   [Json](/docs/api/tools/types/Json)
*   [JsonObject](/docs/api/tools/types/JsonObject)
*   [SerperParameters](/docs/api/tools/types/SerperParameters)